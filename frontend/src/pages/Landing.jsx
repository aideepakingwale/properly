...
export default function Landing() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg-dark)', fontFamily:'var(--font-body)', overflow:'hidden', position:'relative' }}>
      <StarBg count={22} />
      {['🌲','🌳','🌲','🌿','🍃','🌲','🌳'].map((t,i)=>(
      ...
      {/* ── NAV ── */}
      <nav style={{ position:'sticky', top:0, zIndex:50, background:'rgba(13,35,24,0.92)', backdropFilter:'blur(12px)', borderBottom:'1px solid var(--overlay-7)', padding:'12px 24px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:36, height:36, borderRadius:'50%', background:'var(--overlay-10)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>🦉</div>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => { const el=document.getElementById('pricing'); el?.scrollIntoView({behavior:'smooth'}); }}
            style={{ background:'var(--overlay-8)', border:'1.5px solid var(--overlay-15)', borderRadius:50, padding:'6px 14px', color:'rgba(255,255,255,0.75)', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'var(--font-body)' }}>
            Pricing
          </button>
          <button onClick={() => nav('/auth')}
            style={{ background:'var(--grad-cta)', border:'none', borderRadius:50, padding:'6px 16px', color:'white', fontSize:12, fontWeight:800, cursor:'pointer', fontFamily:'var(--font-body)' }}>
            Log in
          </button>
        </div>
      </nav>
      ...
      <div style={{ maxWidth:860, margin:'0 auto', padding:'56px 24px 20px', position:'relative', zIndex:1, textAlign:'center' }}>
        ...
        <h1 style={{ fontSize:'clamp(28px,7vw,62px)', fontWeight:900, lineHeight:1.1, color:'white', marginBottom:18 }}>
          Where Little Readers<br /><span style={{ color:'var(--color-accent)' }}>Find Their Voice</span>
        </h1>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:10 }}>
          ...
          <p style={{ fontSize:12, color:'var(--overlay-30)' }}>No credit card · Ages 4–7 · UK Phonics Phases 2–6</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth:860, margin:'0 auto', padding:'44px 24px 20px', position:'relative', zIndex:1 }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))', gap:16 }}>
          {FEATURES.map((f,i)=>(
            <div key={f.t} className="animate-slide-up" style={{ animationDelay:`${i*0.08}s`, background:'var(--overlay-6)', backdropFilter:'blur(10px)', borderRadius:20, padding:'22px 18px', border:'1px solid var(--overlay-10)' }}>
      ...
      <div id="pricing" style={{ maxWidth:900, margin:'0 auto', padding:'60px 24px 20px', position:'relative', zIndex:1 }}>
        <div style={{ textAlign:'center', marginBottom:32 }}>
          <p style={{ color:'var(--overlay-40)', fontSize:11, fontWeight:800, letterSpacing:'1.5px', marginBottom:8 }}>PRICING</p>
          <h2 style={{ fontSize:'clamp(22px,5vw,38px)', fontWeight:900, color:'white', margin:'0 0 12px' }}>
            Simple, honest pricing
          </h2>
          <p style={{ fontSize:15, color:'var(--overlay-50)', maxWidth:420, margin:'0 auto' }}>
        ...
        <p style={{ textAlign:'center', marginTop:28, color:'var(--overlay-20)', fontSize:12 }}>
      ...
      <div style={{ maxWidth:600, margin:'0 auto', padding:'48px 24px 20px', textAlign:'center', position:'relative', zIndex:1 }}>
        ...
        <p style={{ color:'var(--overlay-50)', fontSize:15, marginBottom:24 }}>Join families already reading with Properly — free forever, no card needed.</p>
      ...
      <div style={{ maxWidth:860, margin:'0 auto', padding:'12px 24px 20px', position:'relative', zIndex:1 }}>
        <p style={{ textAlign:'center', color:'var(--overlay-15)', fontSize:12 }}>
