/**
 * @file        Pricing.jsx
 * @description Subscription pricing page — plan comparison and Stripe Checkout integration
 * @module      Pages
 *
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-muted)', fontFamily: 'var(--font-body)', display: 'flex', flexDirection: 'column' }}>
      <header style={{ background: 'white', borderBottom: '1px solid var(--border)', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => nav(-1)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: 'var(--text-muted)', padding: 0 }}>←</button>
        <span style={{ fontSize: 16 }}>🦉</span>
        <span style={{ fontWeight: 900, fontSize: 16, color: 'var(--text)' }}>Choose a plan</span>
        {sub && sub.plan !== 'free' && (
          <button onClick={handleManageBilling} style={{ marginLeft: 'auto', background: 'none', border: '1.5px solid var(--border)', borderRadius: 50, padding: '5px 14px', fontSize: 12, cursor: 'pointer', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', fontWeight: 600 }}>
            Manage billing →
          </button>
        )}
      </header>
      ...
      <main style={{ flex: 1, maxWidth: 960, margin: '0 auto', padding: '36px 20px 0', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <h1 style={{ fontSize: 28, fontWeight: 900, color: 'var(--text)', margin: '0 0 10px' }}>
        ...
        {pageLoading
          ? <div style={{ textAlign: 'center', padding: 60, color: 'var(--text-light)' }}>Loading plans…</div>
          : <PlansSection currentPlan={currentPlan} showCTA={true} dark={false} />
        }

        {/* Current plan status banner */}
        {sub && sub.plan !== 'free' && (
          <div style={{ marginTop: 24, background: 'var(--bg-primary-light)', border: '1.5px solid var(--border-primary-light)', borderRadius: 16, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 22 }}>✅</span>
            <div>
              <p style={{ margin: 0, fontWeight: 800, color: 'var(--text-success-dark)', fontSize: 14 }}>
                You're on the {sub.planName} {sub.planEmoji} plan
              </p>
              {sub.currentPeriodEnd && (
                <p style={{ margin: 0, fontSize: 12, color: 'var(--text-success)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 20, margin: '28px 0', flexWrap: 'wrap' }}>
          {['🔒 Secure Stripe payments', '↩ Cancel any time', '📧 Email support', '🇬🇧 UK pricing'].map(b => (
            <span key={b} style={{ fontSize: 12, color: 'var(--text-light)', fontWeight: 600 }}>{b}</span>
          ))}
        </div>

        {/* FAQ */}
        <div style={{ maxWidth: 560, margin: '0 auto', paddingBottom: 48 }}>
          <h2 style={{ fontSize: 18, fontWeight: 900, color: 'var(--text)', marginBottom: 20, textAlign: 'center' }}>Common questions</h2>
          {[
            ...
            <div key={q} style={{ borderBottom: '1px solid var(--border)', padding: '14px 0' }}>
              <p style={{ fontWeight: 700, color: 'var(--text)', margin: '0 0 4px', fontSize: 14 }}>{q}</p>
              <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: 13, lineHeight: 1.6 }}>{a}</p>
            </div>
          ))}
        </div>
      </main>
